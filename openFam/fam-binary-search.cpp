...
void populateSearchValues(int lower, int upper, uint64_t searchList[]){
        srand( (static_cast<unsigned int>(time(0)) * myPE));
...
bool binary_search(uint64_t key, Fam_Descriptor *item) {
        ...
        uint64_t *local = (uint64_t *)malloc(sizeof(uint64_t));
        low = 0;
        ...
        while(low < high){
                ...
                */
                ...
                my_fam->fam_get_blocking(local, item, offset, sizeof(uint64_t));
                val = *local;
                if(val == key){
                        return true;
                        free(local);
...
int main(){
        ...
        if(myPE == 0){
                uint64_t size = (4 * 1024UL * 1024UL * 1024UL * 1024UL);
